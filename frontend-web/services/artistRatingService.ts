...
// What collectors think of an artist.
//
...
export const artistRatingService = {
  ...
  listRatingsByUserId: async (userIds: string[]): Promise<Record<string, ArtistRating>> =>
    Object.fromEntries(userIds.map((userId) => [userId, summarizeRating(artistIdFromUserId(userId), [])])),
};
