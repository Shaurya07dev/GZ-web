...
export function CheckoutConfirmStep({
  ...
  if (placedOrder) {
    const total = placedOrder.amount + placedOrder.gstAmount + placedOrder.deliveryCharge;
    return (
      <div className="flex flex-col items-center gap-5 py-6 text-center">
        <span className="flex size-14 items-center justify-center rounded-full border border-gold/40 bg-gold/10">
          <CheckCircle2 className="size-7 text-gold-bright" strokeWidth={1.75} />
        </span>
        <div>
          ...
          <p className="mt-1.5 max-w-sm text-sm text-muted-foreground">
            &ldquo;{artwork.title}&rdquo; is on its way to {address.city}. We&rsquo;ll keep
            you updated as it moves through packing and dispatch.
          </p>
        </div>
        <p className="text-sm text-muted-foreground">
          Total paid <span className="font-medium text-foreground">{formatINR(total)}</span>
        </p>
        <div className="mt-2 flex flex-col gap-3 sm:flex-row">
          <Button render={<Link href={`/account/orders/${placedOrder.id}`} />}>
            View order
          </Button>
    ...
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          You&rsquo;re buying &ldquo;{artwork.title}&rdquo;, delivered to {address.line1},{" "}
          {address.city}. There&rsquo;s no payment form in this preview build; confirming
          places the order at the price shown in review.
        </p>
      </div>

      <div className="flex items-center justify-between border-t border-border pt-5">
        <Button variant="outline" onClick={onBack} disabled={createOrderMutation.isPending}>
